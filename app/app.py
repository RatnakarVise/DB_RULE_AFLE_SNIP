...
class Unit(BaseModel):
    ...
    end_line: int = 0
    code: str
    amount_findings: Optional[List[Finding]] = None  # output, filled by analyzer
...
DECL_PACKED         = re.compile(
    r"\b(DATA|CONSTANTS|PARAMETERS|STATICS)\b[^.\n]*?\b(\w+)\b\s*TYPE\s+P\b[^.\n]*?(?:LENGTH\s+(\d+))?[^.\n]*?(?:DECIMALS\s+(\d+))?",
    re.IGNORECASE
)
DECL_DEC_TYPE       = re.compile(
    r"\b(DATA|CONSTANTS|PARAMETERS|STATICS)\b[^.\n]*?\b(\w+)\b\s*TYPE\s+DEC\b[^.\n]*?(?:LENGTH\s+(\d+))?[^.\n]*?(?:DECIMALS\s+(\d+))?",
    re.IGNORECASE
)
DECL_TYPE_GENERIC   = re.compile(
    r"\b(DATA|PARAMETERS|STATICS)\b[^.\n]*?\b(\w+)\b\s+(?:TYPE|LIKE)\s+(\w+)\b",
    re.IGNORECASE
)

# Colon header + entries
DECL_HEADER_COLON = re.compile(
    r"""(?imxs)^\s*(DATA|STATICS|CONSTANTS|PARAMETERS)\s*:\s*(?P<body>.*?)\.\s*(?:\"[^\n]*)?$"""
)
...
# Optional arithmetic/operator regex (for clarity)
...
def _parse_struct_block(name: str, body: str, symtab: Dict[str, Dict[str, Any]]):
    ...
    for raw in re.split(r",\s*\n|\n|,", body_norm):
        ...
        # LIKE/TYPE TAB-FLD
        m_like = re.search(
            r"^(\w+)\s+(?:LIKE|TYPE)\s+([A-Za-z_]\w*)-([A-Za-z_]\w*)\b",
            ln,
            re.IGNORECASE
        )
        if m_like:
        ...
        m_p_legacy = re.search(
            r"^(\w+)\s*\((\d+)\)\s*TYPE\s+p\b(?:\s+DECIMALS\s+(\d+))?",
            ln,
            re.IGNORECASE
        )
        if m_p_legacy:
        ...
        m_p = re.search(
            r"^(\w+)\s+TYPE\s+p\b(?:\s+LENGTH\s+(\d+))?(?:\s+DECIMALS\s+(\d+))?",
            ln,
            re.IGNORECASE
        )
        if m_p:
...
def char_too_short(
    ...
    token: str,
    min_len: int = MIN_CHAR_FOR_AMOUNT
) -> Optional[bool]:
...
def dec_too_short(
    symtab: Dict[str, Dict[str, Any]],
    token: str,
    min_digits: int = MIN_DIGITS_FOR_AMOUNT
) -> Optional[bool]:
    dd = ddic_lookup_token(token)
...
def _emit_decl_mirrors(
    ...
    for d in decls:
        ...
        if too_small is True:
            msg = (
                f"Declaration of '{dest_token}' appears too small for AFLE amount "
                f"used in {usage_unit.inc_name}/{usage_unit.name} at line {usage_line}."
            )
            sev = "error"
            sug = (
                f"Widen to AFLE-safe length (≥{MIN_DIGITS_FOR_AMOUNT}, e.g. P LENGTH 23 DECIMALS 2 "
                f"or DDIC element)."
            )
            itype = "DeclarationAFLESizeRisk"
        else:
            msg = (
                f"Declaration of '{dest_token}' capacity unknown for AFLE amount usage "
                f"at {usage_unit.inc_name}/{usage_unit.name} line {usage_line}."
            )
            sev = "info"
            sug = f"Verify declaration supports AFLE (≥{MIN_DIGITS_FOR_AMOUNT} digits)."
...
def scan_unit(
    ...
    for m in MOVE_STMT.finditer(src):
        ...
        if not is_amount_expr_like(symtab, src_exp):
            continue
        ...
        if sev:
            usage = pack_issue(
                ...
                "Moving amount expression into {0} ".format(dest)
                + ("(too small)." if too_small else "(destination capacity unknown)."),
                sev,
                m.start(),
                m.end(),
                (
                    f"Use AFLE-safe type (≥{MIN_DIGITS_FOR_AMOUNT} digits), e.g. "
                    f"P LENGTH 23 DECIMALS 2 or appropriate DDIC element."
                )
            )
    ...
    for m in ASSIGNMENT.finditer(src):
        ...
        if sev:
            usage = pack_issue(
                unit,
                "OldMoveLengthConflict",
                "Assignment from amount expression into {0} ".format(dest)
                + ("(too small)." if too_small else "(type unknown)."),
                sev,
                m.start(),
                m.end(),
    ...
    for m in IF_BLOCK.finditer(src):
        ...
        for c in SIMPLE_CMP.finditer(cond):
            ...
            if not (left_is_amt or right_is_amt):
                continue
            is_lit = (
                bool(re.match(r"^'.*'$", right))
                or right.replace(".", "", 1).isdigit()
                or bool(re.match(r"^'.*'$", left))
                or left.replace(".", "", 1).isdigit()
            )
    ...
    for m in SELECT_INTO.finditer(src):
        ...
        if too_small:
            usage = pack_issue(
                ...
                m.end(),
                (
                    f"Align destination to DB type (AFLE-compliant P/DEC; "
                    f"≥{MIN_DIGITS_FOR_AMOUNT} digits)."
                )
            )
            findings.append(usage)
            _emit_decl_mirrors(
            ...
            findings.append(
                pack_issue(
                    ...
                    m.end(),
                    (
                        f"Verify destination vs DB; use AFLE-compliant type "
                        f"(≥{MIN_DIGITS_FOR_AMOUNT} digits)."
                    )
                )
            )
    ...
    for m in WRITE_TO_STMT.finditer(src):
        ...
        if cshort is True:
            findings.append(
                pack_issue(
                    unit,
                    "WriteToTruncationRisk",
                    f"WRITE TO target {target} may truncate formatted AFLE amounts.",
                    "error",
                    m.start(),
                    m.end(),
                    (
                        f"Increase target CHAR length (≥{MIN_CHAR_FOR_AMOUNT}) "
                        f"or format properly."
                    )
                )
            )
...
# =====================================================================
# Sample-style response layer (like Credit Master rule)
# =====================================================================

def build_response(unit: Unit, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Convert internal AFLE findings into unified 'findings' schema, with:
      - issues_type = 'AFLE-' + original issue_type
      - starting_line / ending_line = issue's line
    """
    findings_out: List[Dict[str, Any]] = []
    for i in issues:
        issue_type_raw = i.get("issue_type") or ""
        issues_type = f"AFLE-{issue_type_raw}" if issue_type_raw else "AFLE-Unknown"
...
def remediate_single(unit: Unit):
    """
    Sample-style single-unit endpoint, same pattern as the credit remediator.
    """
    results = analyze_units([unit])
    first = results[0] if results else {}
    issues = first.get("amount_findings") or []
    return [build_response(unit, issues)]

@app.post("/remediate-array")
def remediate_array(units: List[Unit]):
    """
    Sample-style array endpoint, mirroring the credit rule's /remediate-array.
    """
    analyzed = analyze_units(units)
    output = []
    for u, res in zip(units, analyzed):
        issues = res.get("amount_findings") or []
